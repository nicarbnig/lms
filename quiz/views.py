    ...
    try:
        ...
        if is_faculty_authorised(request, code):
            if request.method == 'POST':
                ...
                return render(request, 'quiz/quiz.html', {'course': course, 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id'])})
    ...
    try:
        ...
        if is_faculty_authorised(request, code):
            ...
            if request.method == 'POST':
                ...
                return render(request, 'quiz/addQuestion.html', {'course': course, 'quiz': quiz, 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id'])})
    ...
    if is_faculty_authorised(request, code):
        ...
        for quiz in quizzes:
            ...
            if quiz.start < datetime.datetime.now():
                ...
                quiz.started = False
            quiz.save()
        return render(request, 'quiz/allQuizzes.html', {'course': course, 'quizzes': quizzes, 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id'])})
    else:
        return redirect('std_login')
...
def startQuiz(request, code, quiz_id):
    if is_student_authorised(request, code):
        ...
        return render(request, 'quiz/portalStdNew.html', {'course': course, 'quiz': quiz, 'questions': questions, 'total_questions': total_questions, 'student': Member.objects.get(student_id=request.session['student_id'])})
    else:
        return redirect('std_login')
...
def quizSummary(request, code, quiz_id):
    if is_faculty_authorised(request, code):
        ...
        total_students = Member.objects.filter(course=course).count()
        for question in questions:
        ...
        students = Member.objects.filter(course=course)
        ...
        context = {'course': course, 'quiz': quiz, 'questions': questions, 'time': time, 'total_students': total_students,
                   'students': students, 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id'])}
