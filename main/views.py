...
from .models import Member, Course, Announcement, Assignment, Submission, Material, Facilitators, Department
...
def is_student_authorised(request, code):
    ...
    if request.session.get('student_id') and course in Member.objects.get(student_id=request.session['student_id']).course.all():
        return True
    else:
        return False
...
def std_login(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Member.objects.filter(student_id=id, password=password).exists():
                request.session['student_id'] = id
                return redirect('myCourses')
            elif Facilitators.objects.filter(faculty_id=id, password=password).exists():
...
def facultyCourses(request):
    try:
        if request.session['faculty_id']:
            faculty = Facilitators.objects.get(
    ...
    try:
        ...
        if is_student_authorised(request, code):
            ...
            context = {
                ...
                'materials': materials,
                'student': Member.objects.get(student_id=request.session['student_id'])
            }
    ...
    if request.session.get('faculty_id'):
        try:
            ...
            assignments = Assignment.objects.filter(
                course_code=course.code)
            materials = Material.objects.filter(course_code=course.code)
            studentCount = Member.objects.filter(course=course).count()
        ...
        context = {
            ...
            'assignments': assignments[:3],
            'materials': materials,
            'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']),
...
def profile(request, id):
    try:
        if request.session['student_id'] == id:
            student = Member.objects.get(student_id=id)
        ...
        try:
            if request.session['faculty_id'] == id:
                faculty = Facilitators.objects.get(faculty_id=id)
...
def addAnnouncement(request, code):
    if is_faculty_authorised(request, code):
        ...
        return render(request, 'main/announcement.html', {'course': Course.objects.get(code=code), 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']), 'form': form})
    else:
        return redirect('std_login')
...
def addAssignment(request, code):
    if is_faculty_authorised(request, code):
        if request.method == 'POST':
            ...
            form = AssignmentForm()
        return render(request, 'main/assignment.html', {'course': Course.objects.get(code=code), 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']), 'form': form})
    else:
        return redirect('std_login')
...
def assignmentPage(request, code, id):
    ...
    if is_student_authorised(request, code):
        ...
        try:
            ...
            submission = Submission.objects.get(assignment=assignment, student=Member.objects.get(
                student_id=request.session['student_id']))
            ...
            context = {
                ...
                'student': Member.objects.get(student_id=request.session['student_id']),
                'courses': Member.objects.get(student_id=request.session['student_id']).course.all()
            }
    ...
    try:
        ...
        if is_student_authorised(request, code):
            ...
            if request.method == 'POST' and request.FILES['file']:
                ...
                submission = Submission(assignment=assignment, student=Member.objects.get(
    ...
    if is_faculty_authorised(request, code):
        try:
            ...
            context = {
                ...
                'assignment': assignment,
                'totalStudents': len(Member.objects.filter(course=course)),
                'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']),
    ...
    if is_faculty_authorised(request, code):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return render(request, 'main/course-material.html', {'course': Course.objects.get(code=code), 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']), 'form': form})
        else:
            form = MaterialForm()
            return render(request, 'main/course-material.html', {'course': Course.objects.get(code=code), 'faculty': Facilitators.objects.get(faculty_id=request.session['faculty_id']), 'form': form})
    else:
...
def guestStudent(request):
    request.session.flush()
    try:
        student = Member.objects.get(name='Guest Student')
...
def guestFaculty(request):
    request.session.flush()
    try:
        faculty = Facilitators.objects.get(name='Guest Faculty')
