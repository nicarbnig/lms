...
from discussion.models import InstructorsDiscussion, MembersDiscussion
...
def context_list(course):
    try:
        studentDis = MembersDiscussion.objects.filter(course=course)
        facultyDis = InstructorsDiscussion.objects.filter(course=course)
        ...
        for dis in discussions:
            if dis.__class__.__name__ == 'StudentDiscussion':
                dis.author = Member.objects.get(student_id=dis.sent_by_id)
            else:
                dis.author = Facilitators.objects.get(faculty_id=dis.sent_by_id)
    except:
    ...
    if is_student_authorised(request, code):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                course = Course.objects.get(code=code)
                try:
                    student = Member.objects.get(student_id=std_id)
                except:
                    return redirect('discussion', code=code)
                MembersDiscussion.objects.create(
    ...
    if is_faculty_authorised(request, code):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                course = Course.objects.get(code=code)
                try:
                    faculty = Facilitators.objects.get(faculty_id=fac_id)
                except:
                    return redirect('discussion', code=code)
                InstructorsDiscussion.objects.create(
